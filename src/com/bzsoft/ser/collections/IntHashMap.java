 ...
 * primitive in keys.
 */
public class IntHashMap<V> extends IntMap<V> implements Serializable {
	...
	private static class AbstractMapIterator<V> {
		...
		protected final IntHashMap<V>	associatedMap;

		protected AbstractMapIterator(final IntHashMap<V> hm) {
			associatedMap = hm;
	...
	private static class EntryIterator<V> extends AbstractMapIterator<V> implements IntMapIterator<V> {

		protected EntryIterator(final IntHashMap<V> map) {
			super(map);
		}
	...
	private static class ValueIterator<V> extends AbstractMapIterator<V> implements Iterator<V> {
		...
		protected ValueIterator(final IntHashMap<V> map) {
			super(map);
		}
	 ...
	 *            or equal to zero.
	 */
	public IntHashMap(int capacity, final float loadFactor) {
	...
	protected final Entry<V> getEntry(final int key) {
	...
	protected Entry<V> createHashedEntry(final int key, final int index, final int hash) {
	...
	protected final Entry<V> removeEntry(final int key) {
		int index = 0;
		Entry<V> entry;
		Entry<V> last = null;
	...
	@Override
	public IntMapIterator<V> intMapIterator() {
